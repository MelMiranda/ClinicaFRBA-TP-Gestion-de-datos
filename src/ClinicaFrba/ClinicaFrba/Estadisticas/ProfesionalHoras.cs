    ...
    public partial class ProfesionalHoras : Form
    {
        ...
        private void btn_buscar_Click(object sender, EventArgs e)
        {
            if (cboAño.Text != "" && cboEspecialidad.Text != "" && cboMes.Text != "")
            {
                ...
                SqlCommand cargar = new SqlCommand("TRIGGER_EXPLOSION.ProfesionalesHoras", ManejadorConexiones.conectar());
        ...
        {
            SqlCommand cargar = new SqlCommand("SELECT Descripcion, Id_especialidad FROM TRIGGER_EXPLOSION.Especialidad UNION SELECT 'Todas', 0", ManejadorConexiones.conectar());
            SqlDataAdapter adapter = new SqlDataAdapter(cargar);
            DataTable table = new DataTable();
            adapter.Fill(table);
